...
class Employee {
  constructor(name="",id="",email="") {
    ...
    this.queryList = [
      ...
      ["id",`Enter ${this.getRole()}'s id:`,"integer"],
    ...
    let questions = this.queryList.map(question => {
      return {
        ...
        validate: async (input) => {
          // check type of input against question[2]
          if (question[2] === "string") {
            return (typeof input === question[2])?true:"Invalid input, enter a string"
          } else if (question[2] === "integer") {
            return (/^-?\d+$/.test(input))?true:"Invalid input, enter an integer."
          }
        }
      };
    })
